...
const Type = connection.define('types', {
  ...
  Gender.hasOne(User, {foreignKey: 'gender_id'}); // Adds gender_id to user table
  Gender.hasOne(Pokemon, {foreignKey: 'gender_id'}); // Adds gender_id to pokemon table
  User.hasMany(Catch, {foreignKey: 'user_id'}); // Adds user_id to catch table
  Pokemon.hasMany(Catch, {foreignKey: 'poke_id'}); // Adds poke_id to catch table
